...
class AUserTaggedStoreKeepsPeopleApartTest(unittest.TestCase):
    """A store may declare that its keys are TAGGED with the user, and
    the engine does the tagging. One flag, so any future store opts in
    the same way (ROADMAP line 21).

    The tag is the UID, taken from `prefs.library_user` directly and
    never through `users.current()` - that mints, and minting from
    inside a favourites read is a surprise nobody asked for.
    """
    ...
    def test_two_users_do_not_see_each_others_entries(self):
        ...
        p = self._prefs()
        one = users.create(p, "Cobalt")
        two = users.create(p, "Sienna")

        p.library_user = one
        keyed_store.release()
        locations.set_favourite(p,"/photos/a.exr", True)
        self.assertTrue(locations.is_favourite(p,"/photos/a.exr"))

        p.library_user = two
        keyed_store.release()
        self.assertFalse(
            locations.is_favourite(p,"/photos/a.exr"),
            "the other user's star is showing - the tag is not keeping "
            "them apart")

        p.library_user = one
        keyed_store.release()
        self.assertTrue(locations.is_favourite(p,"/photos/a.exr"),
                        "switching back lost the first user's star")
    ...
    def test_the_stored_key_carries_the_uid(self):
        ...
        p.library_user = uid
        keyed_store.release()
        locations.set_favourite(p,"/photos/a|b.exr", True)
        with open(os.path.join(p.dir, "favourites.json"),
                  encoding="utf-8") as handle:
            stored = json.load(handle)
        keys = list(stored.get("favourites", {}))
        self.assertTrue(keys)
        for key in keys:
            tag, _sep, rest = key.partition(keyed_store.USER_SEP)
            self.assertEqual(uid, tag, "key %r is not tagged" % key)
            self.assertIn("a|b.exr", rest,
                          "a separator inside the PATH was eaten")

    def test_no_user_stars_nothing_rather_than_a_blank_bucket(self):
        ...
        keyed_store.release()
        locations.set_favourite(p,"/photos/a.exr", True)
        self.assertFalse(locations.is_favourite(p,"/photos/a.exr"),
                         "an entry was filed with no user")
