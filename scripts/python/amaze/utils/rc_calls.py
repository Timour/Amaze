    ...
    for pane_tab in hou.ui.paneTabs():  # type: ignore
        if pane_tab.type() == hou.paneTabType.PythonPanel:
            if pane_tab.label() in _PANEL_LABELS:
                ...
                # First match wins - with two Amaze pane tabs open,
                # continuing routed saves to whichever matched LAST
                # (possibly a hidden one).
                break
    if not panel:
...
def save_cop(node=None) -> None:
    """Called from the node right-click "Save to Amaze" on a COP
...
def save_gradient(node=None) -> None:
    """Called from the node right-click "Save Gradient to Amaze" -
...
def save_code(node=None) -> None:
    """Called from the node right-click "Save Code to Amaze" - grabs
