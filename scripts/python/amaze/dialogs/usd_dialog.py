...
class UsdDialog(QtWidgets.QDialog):
    ...
    def __init__(
        ...
        self.fav = False
        self.name = name or ""
        # House convention (base_dialog): canceled is True until the
        # user explicitly accepts - so Esc and the title-bar X (which
        # bypass the button box's rejected signal entirely) read as
        # canceled instead of silently proceeding with a save.
        self.canceled = True
