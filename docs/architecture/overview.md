...
> **A VALUE CARRIES ITS OWN NAME** (2026-08-03, his framing: it is the
...
| **Designed dialogs** | `ui_helpers.DesignedDialog` is the shell his HTML designs describe, and the one to reuse: a dark header band (icon, subtitle, bold title, kind line) over a body column inset equally both sides, `add_field()` placing the design's uneven gaps and `add_buttons()` the pair that fills the column. Measurements come from a Framer page read with computed styles and are stated once in its docstring. Sizes are in PIXELS and map straight across - he designs in Source Sans 3 because it matches Houdini's UI font - and colours are literal, not theme tokens. First use: the Versions dialog, 2026-08-02. | `helpers/ui_helpers.py` |
...
| **Prefs** | Settings, one shared instance injected into every model. Stored where the OS keeps preferences (`~/Library/Preferences/Amaze` on macOS, `%APPDATA%/Amaze` on Windows, `$XDG_CONFIG_HOME/Amaze` on Linux) — never in the install. | `prefs/prefs.py` → `Prefs`, from `settings.json` |
